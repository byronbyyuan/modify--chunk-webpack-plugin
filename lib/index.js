...
class modifyChunkWebpackPlugin {
    constructor(options){
        this.options = {
            basePath:'', //string
            cssChunk:null,
    ...
    replaceChunk(chunkName,compilation,htmlPluginData){
        let name = compilation.chunks.find(item => item.name === chunkName).files.find(item=>item.indexOf('.js')>-1),
            tag = ['body','head'];
            if(name){
                tag.forEach(key => {
                    htmlPluginData[key] = htmlPluginData[key].map(item=>{
                       if(item.tagName === 'script' && item.attributes.src && item.attributes.src.indexOf(name) > -1){
...
module.exports = modifyChunkWebpackPlugin;
